...
def train_erasing(erase_concept, erase_from, train_method, iterations, negative_guidance, lr, save_path, diffuser, device):
    ...
    for i in pbar:
        with torch.no_grad():
            ...
            erase_text_embeddings = diffuser.get_text_embeddings([erase_concept_sampled[0]], n_imgs=1)
            erase_form_text_embeddings = diffuser.get_text_embeddings([erase_concept_sampled[1]], n_imgs=1)
            ...
            # erase_latents = diffuser.predict_noise(iteration, latents_steps[0], erase_text_embeddings, guidance_scale=1)
            erase_form_latents = diffuser.predict_noise(iteration, latents_steps[0], erase_form_text_embeddings, guidance_scale=1)
        ...
        # loss = criteria(negative_latents, target_latents)
        loss = criteria(erase_latents, erase_form_latents)
        # ------------------------------------------------------------------------------------------------------
    ...
    del erase_text_embeddings, erase_form_text_embeddings
    del latents, latents_steps, erase_form_latents, erase_latents
...
def train_concept_inversion(
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            cc+=1
            if cc == early_step:
                save_path_early = save_path
                save_path_early = save_path_early.split('.')
                save_path_early[-2] = save_path_early[-2]+'_early'
                save_path_early = '.'.join(save_path_early)
                print(save_path)
                print(save_path_early)
                torch.save(diffuser.text_encoder.state_dict(), save_path_early)
