            ...
            if module.__class__.__name__ in ["Linear", "Conv2d", "LoRACompatibleLinear", "LoRACompatibleConv"]:
                if train_method == 'xattn':
                    ...
                    if 'attn2' in module_name:
                        continue 
                elif train_method == 'xattn-kv':
                    # print('ok1',module_name)
                    if not ('attn2' in module_name and ('to_k' in module_name or 'to_v'  in module_name)):
                        # print('ok2',module_name)
                        continue
                elif train_method == 'full_unet':
                    pass
 
                # print('ok3',module_name)
                # print(module_name)
...
# STEREO - Utils #
# Helper function to generate a unique random token
def generate_placeholder_token():
    return "token_" + ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))


def generate_unique_placeholder_token(saved_tokens, iteration):
...
# Function to perform inference and save images
def inference_and_save(generic_prompt, prompt, placeholder_token, saved_tokens, iteration, output_dir, device):
    ...
    # Load the updated UNet and text encoder weights for the current iteration
    unet_weights_path = os.path.join(output_dir, f"erased_unet_iteration_{iteration}.pt")
    text_encoder_weights_path = os.path.join(output_dir, f"ci_attack_text_encoder_iteration_{iteration}.pt")
    diffuser.unet.load_state_dict(torch.load(unet_weights_path))
    diffuser.text_encoder.load_state_dict(torch.load(text_encoder_weights_path))
    ...
    # Perform inference with erased and placeholder tokens
    ...
    with torch.no_grad():
        erased_images = diffuser(
            f"{generic_prompt} {prompt}",
            img_size=512,
            n_steps=50,
            n_imgs=5,
            generator=generator,
    ...
    for i, img in enumerate(erased_images):
        img[0].save(os.path.join(iteration_dir, f"erased_image_{i}.png"))

    with torch.no_grad():
        for token in list(saved_tokens.values()):
            attack_images = diffuser(
            ...
            for i, img in enumerate(attack_images):
                img[0].save(os.path.join(iteration_dir, f"attack_image_placeholder_{token}_{i}.png"))
            torch.cuda.empty_cache()

    print(f"Generated and saved images for iteration {iteration}.")

    del erased_images, attack_images, generator, seed, iteration_dir, prompt, placeholder_token, iteration, output_dir, diffuser
    torch.cuda.empty_cache()


def inference_attack(prompt, saved_tokens, output_dir, unet_ckpt, text_encoder_ckpt, device):
    # Initialize the diffuser model on the specified device
    diffuser = StableDiffuser(scheduler='DDIM').to(device)
    # Ensure tokenizer has the placeholder tokens added during the training process
    ...
    # Load the updated UNet and text encoder weights for the current iteration
    unet_weights_path = os.path.join(output_dir, unet_ckpt)
    text_encoder_weights_path = os.path.join(output_dir, text_encoder_ckpt)
    diffuser.unet.load_state_dict(torch.load(unet_weights_path))
    diffuser.text_encoder.load_state_dict(torch.load(text_encoder_weights_path))
    # Switch model to eval mode for inference
    diffuser.eval()
    ...
    # Perform inference with erased and placeholder tokens
    # Create output directories if they don't exist
    iteration_dir = os.path.join(output_dir, f"eval_ci_iteration")
    os.makedirs(iteration_dir, exist_ok=True)
    ...
    with torch.no_grad():
        for token in saved_tokens:
            ...
            for i, img in enumerate(attack_images):
                img[0].save(os.path.join(iteration_dir, f"eval_ci_attack_image_placeholder_{token}_{i}.png"))
            torch.cuda.empty_cache()

    del attack_images, generator, seed, iteration_dir, prompt, output_dir, diffuser
    torch.cuda.empty_cache()